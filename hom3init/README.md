...
with **zero external crates**. It is the boot and supervision layer of the HOM3
...
**Decision required before you publish.** All three options below keep your
pending patent on the *excluded* methods intact; the difference is how much
control you retain over this component.

| Option | Effect | Patent-safe? |
|---|---|---|
| **MIT** (shipped default) | Truly open; max adoption/credibility; anyone can fork/commercialize. | Yes - no express patent grant. |
| **BSD-3-Clause** | Same as MIT, plus a no-endorsement clause. | Yes. |
| **Source-available (custom)** | Public/readable, you retain control; not OSI "open source"; limits outside contribution. Matches a sovereignty-first posture. | Yes. |
| ~~Apache-2.0~~ | **Avoid.** Its express patent grant can undercut your pending application. | **No.** |

This repo ships with `MIT` in `LICENSE` and `Cargo.toml` as a sensible default.
If you want source-available instead, replace `LICENSE` with your
`Sovereign-Use-Only` text and update the `license` field in `Cargo.toml`.
